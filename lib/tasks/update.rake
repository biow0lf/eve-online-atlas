namespace :update do
  ...
  desc 'Updates sov_structures table'
  task sov_structures: :environment do
    # remove old stations
    ...
    structures = []
    json['items'].each_with_index do |_sys, idx|
      structures << SovStructure.new(
        solarSystemID: json['items'][idx]['solarSystem']['id'].to_i,
        allianceID: json['items'][idx]['alliance']['id'].to_s,
        occupancyLevel: json['items'][idx]['vulnerabilityOccupancyLevel'].to_f,
        structureID: json['items'][idx]['structureID'].to_i,
        structureTypeID: json['items'][idx]['type']['id'].to_i,
        vulnerableStartTime: json['items'][idx]['vulnerableStartTime'],
        vulnerableEndTime: json['items'][idx]['vulnerableEndTime'])
    end
