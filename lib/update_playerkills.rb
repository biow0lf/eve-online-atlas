module UpdatePlayerKills
  def self.update_playerKills
    ...
    data['eveapi']['result']['rowset']['row'].each do |row|
      xmlData["#{row['solarSystemID'].to_i}"] = {:shipKills => row['shipKills'].to_i, :factionKills => row['factionKills'].to_i, :podKills => row['podKills'].to_i}
    end

    solarSystemIDsAll.each do |id|
      if xmlData.key?("#{id}")
        result << Killcurrent.new(solarSystemID: id, shipKills: xmlData["#{id}"][:shipKills], factionKills: xmlData["#{id}"][:factionKills], podKills: xmlData["#{id}"][:podKills], cachedUntil: cachedUntil)
      else
